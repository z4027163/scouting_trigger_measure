...
#        cutnum = cutden + " && pass157==1 "
    ...
    tmc.Draw("drll:mll>>hnummc","("+weightmc+")*("+cutnum+")","goff")
    ...
    tdat.Draw("drll:mll>>hdendat","("+cutden+")","goff")
    tdat.Draw("drll:mll>>hnumdat","("+cutnum+")","goff")
    ...
    heffdat.SaveAs("l1_corrCuts_eff_Data2D_2017.root")
    ...
    heffdat.GetXaxis().SetTitle("#it{m_{#mu#mu}} [GeV]")
    heffdat.GetYaxis().SetTitle("#Delta#it{R_{#mu#mu}}")
    heffdat.GetZaxis().SetTitle("Data trigger efficiency")
    heffdat.GetZaxis().SetTitleOffset(1.2)

    heffdat.SetTitleSize(0.049, "xyz")
    heffdat.SetTitleOffset(0.91, "xy")

    heffdat.SetLabelSize(0.05, "xyz")
    heffdat.SetMarkerSize(2.1)
    heffdat.SetMarkerColor(0)
    heffdat.Draw("colztext")
    heffdat.GetZaxis().SetRangeUser(0.0,1.0)
    c1.Update()

    latex2 = TLatex()
    latex2.SetNDC()
    latex2.SetTextSize(0.42*c1.GetTopMargin())
    latex2.SetTextFont(42)
    latex2.SetTextAlign(31)
    if (opt.YEAR == "2018"):
        latex2.DrawLatex(0.86, 0.915,"61.3 fb^{-1} (13 TeV)")
    if (opt.YEAR == "2017"):
        latex2.DrawLatex(0.86, 0.915,"35.3 fb^{-1} (13 TeV)")
    latex2.SetTextAlign(11)
    #latex2.DrawLatex(0.17, 0.815,"p_{T} > 3 GeV")
    #latex2.DrawLatex(0.17, 0.760,"|#eta| < 1.9")
    latex2.SetTextSize(0.55*c1.GetTopMargin())
    latex2.SetTextFont(62)
    latex2.SetTextAlign(11)
    latex2.DrawLatex(0.12, 0.845, "CMS")
...
#    c3.SetLogx()
    ...
    c3.SaveAs(title + "_ratio.pdf")
    #c3.SaveAs("0p5to1p5_ratio.pdf")

    shutil.copy(os.getcwd() + "/" + title + "_ratio.pdf", os.getcwd() + "/newPlots/" + title + "_ratio.pdf")
    shutil.move(os.getcwd() + "/" + title + "_ratio.pdf", "/eos/user/c/ccosby/www/darkPhotonl1Study/newTrigEffs/" + title + "_ratio.pdf")
    print("https://ccosby.web.cern.ch/ccosby/darkPhotonl1Study/newTrigEffs/" + title + "_ratio.pdf")
