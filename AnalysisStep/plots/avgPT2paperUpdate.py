...
def plot():
    ...
    title = "DST_2DeffPT2_avg"
    title = title.replace(" ", "_")
    title = title.replace(".", "p")

    reso = "0.1"
    massRange = "&& mll >(0.55-"+reso+") && mll < (0.55+"+reso+")"
    #massRange = "&& mll > 0.45 && mll < 0.65"
    
    #drbins = [0.0,0.5,1.0,1.5,2.5, 4.0]
    drbins = [0.00,0.02,0.04,0.06,0.08,0.10,0.12,0.14]


    pt2bins = [5,6,8,10,12,14,16]
    ...
    baseCuts = "((pt1>3 && pt2>3)) && abs(eta1)<1.9 && abs(eta2)<1.9 && passSel==1 && mediumid==1"
    #baseCuts = "(pt1>3 && pt2>3) && abs(eta1)<1.9 && abs(eta2)<1.9 && passSel==1  "
    #baseCuts = "((pt1>3 && pt2>3) || "+twoMuDREta+") && abs(eta1)<1.9 && abs(eta2)<1.9 && numMu == 2 && passSel==1 && mediumid==1"
    #baseCuts = "((pt1>3 && pt2>3) || "+twoMuDREta+") && abs(eta1)<1.5 && abs(eta2)<1.5 "
    #baseCuts = "((pt1>3 && pt2>3) || "+twoMuDREta+") && abs(eta1)<1.5 && abs(eta2)<1.5 "
    #baseCuts = "(pt1>3 && pt2>3 && abs(eta1)<1.5 && abs(eta2)<1.5 && passSel==1 "
    ...
    cutnum_2017 = cutden + " && (pass157==1 || passDouble4p5mass==1 || passDouble4dR==1) "
    cutnum_2018 = cutden + " && (pass157==1 || passDouble4p5mass==1 || passDouble4p5massdR1p2==1) "
    ...
    hdendat_2017 = TH2F("hdendat_2017","hdendat_2017",len(pt2bins)-1,array('d',pt2bins),len(drbins)-1,array('d',drbins))
    hdendat_2017.Sumw2()
    hnumdat_2017 = TH2F("hnumdat_2017","hnumdat_2017",len(pt2bins)-1,array('d',pt2bins),len(drbins)-1,array('d',drbins))
    hnumdat_2017.Sumw2()
    tdat_2017.Draw("drll:pt2>>hdendat_2017","("+cutden+")","goff")
    tdat_2017.Draw("drll:pt2>>hnumdat_2017","("+cutnum_2017+")","goff")
    # 2017 Efficiency
    ...
    hdendat_2018 = TH2F("hdendat_2018","hdendat_2018",len(pt2bins)-1,array('d',pt2bins),len(drbins)-1,array('d',drbins))
    hdendat_2018.Sumw2()
    hnumdat_2018 = TH2F("hnumdat_2018","hnumdat_2018",len(pt2bins)-1,array('d',pt2bins),len(drbins)-1,array('d',drbins))
    hnumdat_2018.Sumw2()
    tdat_2018.Draw("drll:pt2>>hdendat_2018","("+cutden+")","goff")
    tdat_2018.Draw("drll:pt2>>hnumdat_2018","("+cutnum_2018+")","goff")
    # 2018 Efficiency
    ...
    #Weighted Average
    heff_average = TH2F("heff_average","heff_average",len(pt2bins)-1,array('d',pt2bins),len(drbins)-1,array('d',drbins))
    heff_average.Sumw2()
    ...
    heff_average.SetBinContent(6,3,0)
    heff_average.SetBinError(6,3,0)
    ...
    heff_average.GetXaxis().SetTitle("Subleading muon #it{p}_{T} [GeV]")
...
#####
    ...
    latex2.DrawLatex(0.49, 0.815,"#it{p}_{T} > 3 GeV")
    latex2.DrawLatex(0.49, 0.760,"|#it{#eta}| < 1.9")
    latex2.DrawLatex(0.49, 0.705,"0.45 < #it{m}_{#it{#mu#mu}} < 0.65 GeV")
    #latex2.DrawLatex(0.56, 0.647,"No Medium ID")
    #latex2.DrawLatex(0.56, 0.603,"n_{#mu} = 2")
    #latex2.DrawLatex(0.56, 0.647,"Medium ID")
    #latex2.DrawLatex(0.56, 0.603,"n_{#mu} #geq 2")
    ...
    latex2.SetTextAlign(11)
    #latex2.DrawLatex(0.200, 0.915, "Preliminary")
